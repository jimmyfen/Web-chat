...
{
	public static function hook($server, $request)
	{
		Static::checkConnect($server, $request->fd);

		$fd = Fd::hGet();
		$fd = !$fd ? [] : $fd;
		$name = isset($request->get['name']) ? $request->get['name'] : Static::getRandomName();
		$fd[$request->fd] = [ 'name' => $name ];
		Fd::hSet($fd);

		foreach ( $server->connections as $_fd ) {
			if ( $_fd !== $request->fd ) {
				$server->push($_fd, json_encode([ 'command' => 'ADD_USER', 'content' => [ 'fd' => $request->fd, 'name' => $name ] ], JSON_UNESCAPED_UNICODE));
			}
		}
